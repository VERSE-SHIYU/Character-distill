...
- **识别**，242 片，账户上限 18：收敛从 60 起，按「一波一次」下调 60→45→33→24→18 共约 4 波，单个请求最多被 429 约 4 次 < 上限 5；之后 ⌈242/18⌉≈14 轮 × 约 12 s ≈ 170 s，加别名判断约 10 s，**约 3 分钟 < 600 s**。
- **蒸馏宝玉**，约 180 片：上限 18 时 Map ≈ 10 轮 × 约 20 s ≈ 200 s，加 3 批合并 2–3 分钟、4 组格式化约 1 分钟 → 约 6–8 分钟，**超出 5 分钟目标**；上限 ≥ 60 时 Map ≈ 3 轮 ≈ 60 s → 约 3–5 分钟，达标。上限随余额动态变化（上游原文「based on your remaining balance」；实测充值后 60 并发 0 次 429、余额下降后降到 18），**5 分钟目标不变**，验收前提是账户余额足以让上限 ≥ 60；AIMD 保证余额低的用户也能跑完（只是更慢）。
