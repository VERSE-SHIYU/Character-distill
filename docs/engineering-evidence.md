...
| `✗ 不符` | 入库核对时实测与本文原表述**矛盾**。2026-09-11 发现 1 处（`text.py` 403），当日已修复转正——**当前档内已无 `✗` 行**，标记保留备后用 |
...
3. **去标识核对结果**：入库时实测本文**不含**真人同人作品名或 `text_id` / `card_id` 取值（逐项 grep 零命中），故入库轮为零改动。`tests/perf/` 那套占位符（`角色A`…`角色D`）不适用于本文——它谈的是代码路径与统计量，不涉及测试语料身份。
   → **补一处（2026-09-11）**：§五 那行原带「（Shiyu 15 全 2026-05 + testadmin 4）」的**账号名拆解**，改这一行时已**去掉账号名、只留「2 个用户」**。注意其**源头 `docs/384-dim-stale-collections.md` 仍带这两个账号名**，属本轮 grep 清单未覆盖的范围——**只列不修**，见下方「同型未覆盖」。
...
5. **同型未覆盖（在本轮 grep 清单之外，按「只列不修」未动）**：
   - `tests/test_card_guard.py` 里还有**另一个语料角色名**（与本轮点名的两个同属作品人名），本轮只点了名的那两个，故未处理。
   - **账号名 `Shiyu` / `Shiyu_ss` 散在 7 个 tracked 文件**（含 `docs/384-dim-stale-collections.md`、`scripts/*.py`）。其中 `scripts/` 下有一个脚本把**账号 + 口令明文硬编码**（口令值不在本档复述）。这与「仓库是公开作品集」直接冲突，**建议单独立项**；本轮不动。
