...
| `✗ 不符` | 2026-09-11 入库核对时实测，与本文原表述**矛盾**（见「入库时发现」） |
...
3. **去标识**：与 `tests/perf/` 同口径（角色占位符 / 作品名移除 / `text_id` 保留）。**实测本文原本就不含真人姓名、作品名或任何 `text_id` / `card_id` 取值**——此条为零改动，验据见「入库时发现」第 3 条。
...
| **影响面** | 47 个 chroma 集合中 38 个为 384 维；12 个 `text_` 有存活卡引用 → 19 张卡 / 2 个用户受影响，且 web 单卡 chat 同路径同样受影响。 |
...
| **数据修复** | 可重跑脚本完成存量重建：12 集合、2966 分片写入、**2127 次真实 embed**、839 次缓存命中、521s、约 0.26–0.5 元。 |
...
| 数字 | 出处 | 判定 |
|---|---|---|
| 12 个 `text_id` → **19 张卡 / 2 用户**（Shiyu 15 全 2026-05 + testadmin 4） | `docs/384-dim-stale-collections.md`（tracked） | ✅ 仓库文档 |
| **47 个 chroma 集合 / 38 个 384 维** | 仅 `.claude/sessions/2026-09-09-mcp-step23-384audit.md` 与 `2026-09-09-rag384-fix.md`（**均 gitignored**）；仓库内文档只写到「12 → 19」这一层 | ⚠️ 待核 |
...
| **存量重建数据**：12 集合 / 2966 分片 / 2127 次真实 embed / 839 次缓存命中 / 521s / **0.26–0.5 元** | **全仓零出处**：tracked 仓库与 gitignored `.claude/sessions/` 均无这几个数（`2966` / `2127` / `521` / `0.26 元` 逐个 grep 零命中）。**重建脚本本身也不在仓库** | ⚠️ 待核 |
| commits `4a971d1` `1d19beb` `a1982f0` `65950e1` | `git log` 逐个存在 | ✅ |

> 本节是全档**来源质量最弱**的一节：核心影响面（38 个集合）与全部成本数字都无仓库内支撑。若要进简历，建议只保留「12 → 19」这一条（有 `docs/384-dim-stale-collections.md` 兜底），或把审计/重建脚本补入库后逐条转正。
...
| 「返回 **404** 而非 403」 | `voice.py` 的 `voice_synthesize` / `preview_audio` 确为 404 ✅；但 **`text.py` 的 `get_upload_task_status` 实为 `HTTPException(403)`** —— 见下 | ✗ 不符 |
| 「无数字，别凑」 | 与实测一致：本节无前后对照数字 | ✅ |
| commits `a0a3a4d` `cdea9e5` `45895f4` | `git log` 逐个存在 | ✅ |

> **`✗ 不符` 详情**：修复口径那句「返回 404 而非 403」对三个端点**不是统一成立的**。
> - `voice.py` 两处（`voice_synthesize` / `preview_audio`）→ **404** ✅
> - `text.py` 的 `get_upload_task_status` → **403**（`a0a3a4d` 引入，代码在 `web/routers/text.py`）。
>
> 且 `AGENTS.md` 缺陷 9 的正文也写着这三处「统一 fail closed + 非属主 **404**」——**台账与代码在这一处互相矛盾**。`AGENTS.md` §四 的口径是「授权失败一律 404」（理由：403 是存在性枚举的预言机），`text.py` 这个新增端点却用了 403。
> 处置：**只列不修**（不在本次 spec 范围；且它属于「403 存量翻 404」这一类，与缺陷 13 同族，但缺陷 13 记的是「存量约 38 处」，未含这个 9-10 新引入的点）。建议单独裁定是改代码还是改台账。
...
1. **`text.py` 越权修复用了 403，与档/台账的「统一 404」口径矛盾**（详见 §六 `✗ 不符`）。
   `a0a3a4d` 给 `get_upload_task_status` 加的是 `HTTPException(403)`；`AGENTS.md` 缺陷 9 正文却写「统一 fail closed + 非属主 **404**」，档里也写「返回 404 而非 403」。**代码与台账在这一处互相矛盾**，且它是 9-10 新引入的点、不在缺陷 13 记的「存量约 38 处」里。需裁定改代码还是改台账。

2. **§五「数据修复」的整组数字（12 集合 / 2966 分片 / 2127 次真实 embed / 839 次缓存命中 / 521s / 0.26–0.5 元）全仓无出处**，重建脚本也不在仓库。这是缺陷 14（已报数字指向 gitignored 产物）的又一例，且比 `out_v5.json` 更彻底——连 gitignored 的会话记录里都没有。

3. **去标识核对结果**：本文**原本不含**任何真人姓名、真人同人作品名或 `text_id` / `card_id` 取值（逐项 grep 零命中），故这项为零改动。`tests/perf/` 那套占位符（`角色A`…`角色D`）不适用于本文——它谈的是代码路径与统计量，不涉及测试语料身份。
...
- [ ] §五 的审计/重建脚本与成本数字补入库（或从简历中删去该节成本数字）
- [ ] §六 的 403/404 口径矛盾裁定（改代码 or 改台账）
