...
F. 人际关系：覆盖前置步骤中枚举的【所有】有名字角色，逐一独立成条，不要遗漏次要角色（前任/现任/配角），不要把不同角色合并为一条，不要列入宠物等非人物。每条给出关系类型和态度，态度需体现情感变化。
...
列表字段（personality_traits/values/key_memories/inner_tensions/emotional_patterns/character_arc）的每个元素必须是【一句字符串】，不是对象。把结论和关键依据（含出处）浓缩进一句话，出处用括号附句尾。严禁输出 {trait:..., description:...} 这种嵌套对象。
"""
