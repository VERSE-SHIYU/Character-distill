...
**为什么要有这个构造**：报错靠 `logging` 走出去 —— 根日志器上的 stdout handler
（`core/stdout_logging`，装设在 `web/server.py` 的 lifespan 里）收 WARNING+，Sentry 的
LoggingIntegration 把 ERROR 变成 GlitchTip 事件。而各处手写的 `try/except + print`
到不了那里 —— SG「全员用量为 0 数月无人察觉」就是这个形态：每一次写入失败都只落在容器
stdout 里，GlitchTip 上一片干净。同一个意图（失败不影响主流程）在仓里被手写了十来遍，
于是「沉默」也被手写了十来遍。
...
**为什么带 `source` / `what`**：归并后的一条问题只有一行文本。读的人需要知道是谁、在哪
一步失败 —— `source` 是子系统名（`"chat"`），`what` 是动宾短语（`"save assistant
message"`），都不是函数名。异常类型与消息由本构造拼进同一行，因为 GlitchTip 按
**消息模板**归并，只拿得到 `getMessage()`。
"""
...
def _report(exc: BaseException, source: str, what: str, level: int) -> None:
    ...
    消息用 `%s` 占位而不是 f-string：GlitchTip 按**消息模板**归并，
...
async def nonfatal(
    ...
    """吞掉块内异常，并以一条 ERROR 上报（生成 GlitchTip 事件）；结果对象告知调用方成没成。

    不带 `as` 的写法（`async with nonfatal(...)`）照旧可用 —— 不关心结果的就别接。

    **`level` 是关键字参数，默认不变（ERROR = 会发告警邮件）。** 口径只有两档：
    「数据没存进去 / 用户的请求失败了」记 ERROR；「已经兜底、不影响结果的后台动作」
    （好感度评估、阅读进度、预热、缓存）记 WARNING —— 后者仍落 stdout，但不发信、也不
    生成 GlitchTip 事件。写成 ERROR 的代价是真会给人发邮件，所以默认档必须留在「这次真的
    没成」上。
