...
# visibility = 'public'` 都是这份关系的第二份写法。PG 侧同形（只差 `$n` 占位符）。
...
def _published_copy_of(copy_ref: str, draft_ref: str) -> str:
    ...
    两侧 `copy_ref` / `draft_ref` 都可以是别名、表名，或在外层被 `?` 绑定的行 —— 谓词只看
...
class SQLiteStore(StorageBase):
    ...
    async def save_card_avatar(self, card_id: str, user_id: str, avatar_data: str) -> None:
        ...
        **有意用关系而非在架谓词**：下架只是不再对市场可见，不该把草稿与副本的头像同步断开。
        ...
        try:
            async with await self._connect() as conn:
                await conn.execute(
                    "UPDATE cards SET avatar_data = ? WHERE id = ? AND user_id = ?",
                    (avatar_data, card_id, user_id),
                )
                # 向下：本卡的发布副本（引用关系定义，副本行 = cards，草稿行 = 本卡）
                await conn.execute(
                    f"UPDATE cards SET avatar_data = ? WHERE EXISTS (SELECT 1 FROM cards d"
                    f" WHERE d.id = ? AND d.user_id = ? AND {_published_copy_of('cards', 'd')})",
                    (avatar_data, card_id, user_id),
                )
                # 向上：本卡是某张草稿的发布副本时，回写那张草稿（引用同一关系定义，方向相反）
                await conn.execute(
                    f"UPDATE cards SET avatar_data = ? WHERE EXISTS (SELECT 1 FROM cards c"
                    f" WHERE c.id = ? AND c.user_id = ? AND {_published_copy_of('c', 'cards')})",
                    (avatar_data, card_id, user_id),
                )
                await conn.commit()
        except Exception as exc:
    ...
    async def get_group_session_unscoped(self, id: str) -> dict | None:
        ...
        **仅供管理员逃生口**：`core/authz.fetch_for_actor` 在属主取不到、且调用方
        ...
        try:
            ...
            print(f"[SQLiteStore] Get group session (unscoped) failed: {exc}")
            raise
    ...
    async def get_session_reactions_owned(self, session_id: str, user_id: str) -> dict[int, list]:
        ...
        Ownership is filtered in SQL (JOIN sessions, WHERE s.user_id = ?). This
        ...
        try:
            ...
            print(f"[SQLiteStore] Get session reactions failed: {exc}")
            raise
    ...
    async def _get_group_reactions(self, group_id: str) -> dict[int, list]:
        ...
        try:
            ...
            print(f"[SQLiteStore] Get group reactions failed: {exc}")
            raise
    ...
    async def get_text_comments_owned(self, text_id: str, user_id: str, page: int = 1, page_size: int = 20) -> dict:
        ...
        try:
            ...
            async with await self._connect() as conn:
                cursor = await conn.execute(
                    """SELECT COUNT(*) FROM text_comments tc JOIN texts t ON t.id = tc.text_id
                       WHERE tc.text_id = ? AND tc.parent_id = '' AND t.user_id = ?""",
                    (text_id, user_id),
                )
                row = await cursor.fetchone()
            ...
            print(f"[SQLiteStore] Get text comments (owned) failed: {exc}")
            raise
    ...
    async def get_card_versions_owned(self, card_id: str, user_id: str) -> list[dict]:
        ...
        try:
            async with await self._connect() as conn:
                cursor = await conn.execute(
                    """SELECT cv.id, cv.card_id, cv.user_id, cv.version_num, cv.publish_message, cv.diff_json, cv.card_json_snapshot, cv.created_at
                       ...
                       WHERE cv.card_id = ? AND c.user_id = ?
            ...
            print(f"[SQLiteStore] Get card versions (owned) failed: {exc}")
