...
- **角色撤回**：会话级状态机（冷却轮次 + 单会话上限 + LLM 人格判断）决定角色是否后悔撤回，前端显示"对方撤回了一条消息"，点击可偷看原文
- **网络搜索增强**：对话时可选开启现实信息补充，上下文引入实时网络内容
- **Agent 工具调用**：agent_mode 下场景/记忆/联网三路检索由 LLM 自主决策调用（详见"Agent 工具调用"章节）
- **拟真交互细节**：重逢开场白（打字机+时区感知频控）、离线时刻注入（朋友+关系）、每日到访感知、沉默机制（三闸判断）、离场感知、心情驱动打字速度、口癖渗透（jieba 分词）
...
对话中三路检索（场景原文 / 长期记忆 / 联网信息）由 LLM 通过 function-calling
自主决策是否调用（ReAct 循环），替代原先每一轮无条件注入全部上下文的做法。

### 架构：决策与生成解耦

为避免角色人设 prompt 压制工具调用，工具决策与回复生成拆为两阶段：
...
用户消息
   ...
   ▼
┌─ 阶段一：工具决策 ─────────────────────────────┐
│  中性 Router prompt + 截断角色背景（≤300 字符）   │
│  → LLM 自主决定是否调 search_memory / search_   │
│    scenes / web_search                          │
│  → 支持多工具同轮调用                             │
│  → MAX_STEPS=3 轮，同参自动去重                   │
└──────────────────────┬─────────────────────────┘
                       │ 检索结果
                       ▼
┌─ 阶段二：回复生成 ─────────────────────────────┐
│  完整角色 system prompt + 【检索参考】块          │
│  → LLM 以角色身份回复（流式，与 legacy 路径同构）  │
│  → 超出动态区预算时从末尾截断                     │
└──────────────────────┬─────────────────────────┘
                       │
                       ▼
...
决策轮使用中性 Router prompt（**ROUTER_SYSTEM_PROMPT**），只附角色背景前 300
字符作为 hint，避免完整角色 persona 使模型产生"我自己知道"的错觉而跳过检索。
生成轮将检索结果以 `【记忆】/【场景】/【联网】` 标签块回注角色 prompt，供模型
参考后回复，不留 tool_calls / tool 痕迹。

### 工程保障

| 机制 | 值 | 说明 |
|------|-----|------|
| MAX_STEPS | 3 | 最多 3 轮工具决策循环 |
| 单工具超时 | 5s / 5s / 15s | 场景/记忆/联网 分别独立超时 |
| 失败降级 | ToolsNotSupportedError | provider 不支持 tools 时自动回退 legacy 无条件注入 |
| 同参去重 | (tool, args) 二元组 | 连续相同调用跳过执行，直接返回去重提示 |
| 结构化日志 | [AgentTool] / [AgentLoop] | 每步执行耗时、工具名、ok 状态、调用参数 |

### 评测（离线）

40 例三类评测集（[scripts/run_agent_eval.py](scripts/run_agent_eval.py)）：
...
> 以下为 legacy 默认路径；开启 agent_mode 时，步骤 1/2/3/5 的检索改由 LLM 通过 function-calling 自主决策调用（见"Agent 工具调用"章节），其余步骤一致。
...
8. **撤回判断**：会话级状态机（冷却 + 上限闸门通过后）触发轻量 LLM 调用，判断角色是否后悔这句话
...
- [x] 手动长期记忆管理
- [x] Agent 工具调用循环（ReAct + 决策/生成解耦 + 40 例评测集）
- [x] 拟真交互细节包（重逢/离线时刻/沉默/离场/打字速度/口癖渗透）
- [ ] 多角色同时蒸馏
- [ ] 微信公众号接入
