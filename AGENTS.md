...
**71. web/routers 下就地 `except Exception` 只让 LLM 失败的两处放行，其余拒绝变 500** —— 状态：**部分修复**（2026-09-24，Spec 71 A-2 首段：`1ca2329` + `97c2934`）
...
- **未覆盖的量**：`chat.py`（3）+ `history.py`（12）＝ **15 处**，属缺陷 96 的改动面，本批未碰；连同**全量 route lock**（禁新增未包装的 5xx 包装；那 1 处 503 登记为 `policy_table` 豁免，理由「缺陷 C 的契约：落库失败拒绝启动 → 503，已记 ERROR」）一起归**收尾段**。**本批没有 route lock** —— 只有两条新用例钉住两个代表点，别把这句读成「锁已就位」。
