...
> **本行的重算触发条件（2026-09-17 记）**：本行是 **2026-09-15** 的快照，今天已明显滞后 —— 现跑现数：带状态行的条目已 **54** 条（本行只算到 46），且桶全变了 —— 已修 **46**（本行写 34）、记账 **3**（35 / 36 / 47；本行写 7）、记账待补 **0**（30 已于 `885735c` 收口）、纵深防御 1（3）、已移出 1（10）、已裁定 1（31）、证伪 1（37）、环境事实仍在 1（50）。**待办 = 记账 3**（本行写 8）。**不逐条订正**：本轮批次里的 30 / 54 尚未收口，今天改完明天又滞后。**触发条件 = 本轮收口批次（30 / 43 / 54）全部走完之后，一次性重算整行**；在那之前，本行只能当「2026-09-15 那一刻的记录」读，任何「还剩几条 / 某条什么状态」一律走下一行的现数配方。
