...
**54. 入库产物 `tests/perf/*_red_lines.json` 的坐标会陈旧，而陈旧时**红在错的地方**** —— 状态：**记账（不修）**（2026-09-17 缺陷 41 名单式落地时撞上，与缺陷 45 同族）
...
- **同一事实的正面余波（已做）**：名单的键因此改成**判别器文本 + 同文本第几处**（`lock_coverage.gap_keys()`），名单本身不再随插行平移 —— 但**产物**仍是行号坐标，本条不受影响、仍然成立。
- **处置方向（留档，本轮不实现）**：① 产物里记一个「生成时对应哪个树」的指纹（如三个靶子文件的 sha256 摘要），`test_lock_coverage.py` 先核指纹再核集合 —— 这样陈旧会**红在它自己的名字上**；② 或在 CI 里跑驱动（成本是真问题：每轮按分钟到小时计，且要先满足 docker / 真服务 / 真 PG，见缺陷 45）；③ 或不入库、本地现算（那就没有可追溯的红源证据，违背 §四「证据产物必须入库」）。**三条各有代价，本轮不做选择。**
