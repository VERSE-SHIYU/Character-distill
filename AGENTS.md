...
**126. 用户向量检索 API key 明文进日志；embedder 缓存按 key 前 8 位区分** —— 状态：**已修**（`9717993` + `86e7aab`，2026-09-25；72 线 · 向量检索密钥，spec `docs/specs/72-embedding-key-leak.md`）
- **归属与顶替**：72 线。本条顶替同线已**证伪**的「库重启后首个请求 500」—— 取证：两次 500 都落在库已停止、尚未启动的窗口内（`socket.gaierror` 主机名解析失败），三种重启方式共 9/9 次请求全 200，不是缺陷，故不修。
- **形态（修前）**：`core/indexing_service.py:45` 把缓存身份拼成 `<text_id>:<完整 key>`，`:48` / `:50` 每次取 RAG 都把**这整串**打到 stdout（运行时容器日志里实见 `[RAG] cache HIT text_…：sk-…`）—— 它进的就是 `docker logs`。同类的身份问题在 `core/embeddings.py:438`：工厂缓存的键只取 key 的前 8 位，而 `sk-` 之后只有 5 个随机字符。
- **为什么是缺陷**：两条各自独立。① 凭据进日志 —— 日志是长期留存面（采集 / `docker logs`），而写这条日志的路径**每次**取 RAG 都走；② 两个不同 key 只要前 8 位相同就**共用同一个 `DashScopeEmbedding`**，后者的请求用前者的 key 发出、记在前者账上 —— 静默串号，日志和账单上都看不出是谁的。
- **修法（`9717993`）**：指纹只有**一处**定义 —— `core/embeddings.py` 新增 `key_fingerprint(key)`（SHA-256 前 16 位十六进制；空串原样返回空串，保住「没配 key」的显式语义）。两处缓存的身份都改用它，日志只留 `text_id` / region 这类非敏感字段。**缓存语义不变**（仍按「书 + key」、「region + key」区分），只换身份的表示方式。
- **锁（`86e7aab` 及同批单元级）**：`tests/test_secret_never_plaintext.py` 的两个金丝雀（SQLite / PG）原来只扫库、**守不住日志**；现把这次链路吐出的 stdout 与日志记录一并收进来，断言 canary 凭据本体、及其**任意 8 位以上子串**都不出现。金丝雀链改用真 `IndexingService`（`RAGEngine` 替身只挡 chroma / 出网），于是 `_ensure_session` 恢复会话那段真的走 `_get_or_build_rag` —— 这正是缺陷的发生点；另加一条「恢复会话必须真取到 RAG」的前置断言，挡掉「夹具没跑起来 → 扫描全绿」的假绿。工厂缓存身份另有一条在 `tests/test_embeddings.py::TestFactoryCacheIdentity`（异 key → 不同实例；同 key → 仍复用，锁住语义没被改坏）。
- **变异实测**：① 把 `indexing_service.py` 改回「完整 key 进缓存身份并打进日志」⇒ 单元级那条 + 两个金丝雀**三条同红**（实测命中行即 `[RAG] cache MISS …：emb-canary-…`）；② `embeddings.py` 改回「只取 key 前 8 位」⇒ `TestFactoryCacheIdentity` 的异 key 那条**红**（实测 `a is b`，同一个对象）。逐字节还原后全绿。
- **判据命令**：`git grep -n "cache HIT {cache_key}\|api_key\[:8\]" -- core web adapters storage` 应给 **0 行**；`git grep -n "def key_fingerprint" -- core` 应给 **1 行**（指纹只有一处定义）。两条都**限定代码目录**：本条正文按规矩要引这两个标识符以记录被删机制，不限范围会自匹配 `AGENTS.md` 自己 —— 命中数不是判据，「代码里还有没有这两处写法」才是。
- **上线后（服务器侧，本段不执行）**：生产两台旧 app 容器的 `docker logs`（json-file）里**仍留着明文 key**；正常部署会重建 app 容器，旧容器连同它的日志一并删除，届时自动清掉。部署后 SZ、SG 各核验一次：`docker logs character-distill-app-1 2>&1 | grep -c "cache HIT .*:sk-"` 应为 **0**（这是匹配**模式**，不是凭据本身），并确认旧 app 容器已不存在（`docker ps -a`）。
