...
> **全表状态口径（2026-09-15 现跑现数）**：1–40 共 40 条 —— **已修 30** / **记账 4**（32、33、35、36）/ **记账待补 1**（30）/ **已裁定 1**（31）/ **进行中 1**（40：commit 一 已落、commit 二 待做）/ 其余 3 条（3 纵深防御、10 已移出「三之二」、37 证伪）。**待办 = 记账 + 记账待补 + 进行中 = 6**。
...
**40. 依赖只有下界 ⇒ 上游发版能改本仓测试的结论，且它与「本仓写错了」共用一个红；同一端点「多大算太大」分散在三处** —— 状态：**commit 一 已落、commit 二 未做**（2026-09-15）
...
- **修法（commit 二：消灭「两条入口两套约束」）**：**待做**。先 grep `text` 表单字段的真实调用方（`web/frontend/` / `mcp_server/` / `scripts/` / tests / README / 接口说明），把命令与结果写进 commit message，再二选一 —— **零调用方 → 下掉 `text` 字段**（那条用例随被测对象一起消失，不是改绿是它不该存在）；**有调用方 → 约束前移**（route 层先判长度、给本仓文案，文案进 `core/text_failure.py`；**不许在框架报错上包一层翻译**）。无论哪条都要回答「修完之后『多大算太大』还剩几处」，目标一处。
...
- **未做 / 已知代价（如实记）**：① **本地 venv 与锁差 86/156 条**（`openai` 2.24.0→3.13.0、`starlette` 1.0.0→1.6.0、`gradio` 6.14→6.27 …）—— 本地跑全量前理论上该按锁重装，否则那条 xfail 会 XPASS 成红（这是**设计使然**：它报的正是「你的环境不是锁的那套」）。**但本机现在不能这么装**：锁里的 `onnxruntime==1.30.0` 在这台 Windows 上加载不了 DLL，照锁重装后本地全套会在中途段错误（单变量反向对照见 §五 环境账）—— 这是本 commit 的一个**真实本地代价**，与 CI 无关（CI 是 Linux）。② **「`.in` 改了忘了重锁」目前没有门**：本可加 `uv pip compile … -c requirements.txt` 对账，但那个步骤要在 gate 里连网解析最新 —— 一旦解析侧出问题，gate 就会因为**世界的原因**变红，正好复犯本轮要修的错（两种成因共用一个信号），故**故意不加**；重锁命令已写进 `.in` 头部。③ **不建第三条 verdict job**：job 命名 + 注解 + 产物已构成契约。
