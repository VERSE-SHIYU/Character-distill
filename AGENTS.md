...
- **形态锁 `tests/test_text_failure_messages.py`（5 条，不维护点位白名单）**：
  ...
  - **L5（缺陷 40 commit 二 加；2026-09-15 缺陷 42 第 3 步把它从 AST 形状层迁到框架账本层）**：**全仓每一条**声明了表单 content 的 operation，正文只能走 `file`，其余 Form 字段只能是元数据（`_FORM_METADATA` 按 `(path, method)` 键，四条 op 各一行）。判据读的是 `route_facts.form_fields(...)`（`get_openapi` 出的文档 + `$ref` 解析），**不再读 `Form(...)` 默认值的形状** —— 旧判据同时漏掉三种等价写法（`Annotated[...]` / keyword-only / `fastapi.Form`），且只锁得住它硬编码那一处 `web/routers/text.py::upload_text`。覆盖面 1 条 → 4 条是**能力达到，不是扩面**（命题本来就适用于所有表单 op，旧判据扫不动别的才只锁一条）。负控 `test_l5_scan_is_not_vacuous_and_policy_has_no_stale_entries`（表单 op 集合非空 + `file` 字段**真的存在** + `_FORM_METADATA` 无陈旧条目）。**边界（如实写在 docstring 里）**：元数据表那半边是**意图不是事实**（哪几个字段算元数据没有事实层对应物），失效方向是**响亮误伤**（新增字段就红、红的信息点名该改哪一行），故不配绕过型变异。
...
**42. 判据读的是「看起来像」而不是事实 —— 同一形态三处显形：`Form(...)` 的 AST 形状 / 并列身份按模块名 / 按 `__file__` 字符串** —— 状态：**已修**（第 1 步 `7009d77` + `3e2670d` + `2c9fee9`，第 3 步 = 本条目所在提交，2026-09-15）
