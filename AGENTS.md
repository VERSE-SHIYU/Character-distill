...
> **全表状态口径（2026-09-15 现跑现数）**：1–41 共 41 条 —— **已修 33**（含 32、33；40：commit 一 `53bed63` + commit 二）/ **记账 3**（35、36、41）/ **记账待补 1**（30）/ **已裁定 1**（31）/ 其余 3 条（3 纵深防御、10 已移出「三之二」、37 证伪）。**待办 = 记账 + 记账待补 = 4**。
...
- **形态锁 `tests/test_text_failure_messages.py`（5 条，不维护点位白名单）**：
  ...
  - **L5（缺陷 40 commit 二 加；2026-09-15 缺陷 42 第 3 步把它从 AST 形状层迁到框架账本层）**：**全仓每一条**声明了表单 content 的 operation，正文只能走 `file`，其余 Form 字段只能是元数据（`_FORM_METADATA` 按 `(path, method)` 键，四条 op 各一行）。判据读的是 `route_facts.form_fields(...)`（`get_openapi` 出的文档 + `$ref` 解析），**不再读 `Form(...)` 默认值的形状** —— 旧判据同时漏掉三种等价写法（`Annotated[...]` / keyword-only / `fastapi.Form`），且只锁得住它硬编码那一处 `web/routers/text.py::upload_text`。覆盖面 1 条 → 4 条是**能力达到，不是扩面**（命题本来就适用于所有表单 op，旧判据扫不动别的才只锁一条）。**覆盖面由 `route_facts.form_operations()` 给**（2026-09-15 缺陷 42 第 3b 步：此前 L5 自己读 `route_facts._FORM_CONTENT_TYPES` / `_METHODS` 重算了一遍 —— 跨层读私有常量，且同一判定两份实现；现已整段删除，content-type 判定只留在事实层的 `_form_media` 一份）。负控 `test_l5_scan_is_not_vacuous_and_policy_has_no_stale_entries`（表单 op 集合非空 + `file` 字段**真的存在** + `_FORM_METADATA` 无陈旧条目）。**边界（如实写在 docstring 里）**：元数据表那半边是**意图不是事实**（哪几个字段算元数据没有事实层对应物），失效方向是**响亮误伤**（新增字段就红、红的信息点名该改哪一行），故不配绕过型变异。
...
**42. 判据读的是「看起来像」而不是事实 —— 同一形态三处显形：`Form(...)` 的 AST 形状 / 并列身份按模块名 / 按 `__file__` 字符串** —— 状态：**已修**（第 1 步 `7009d77` + `3e2670d` + `2c9fee9`，第 3 步 `fbb9066`，第 3a 步 `efa36a6`，第 3b 步 = 本条目所在提交，2026-09-15）
- **病灶的通用形态**：判据与被判事实之间**隔着的东西**，就是能骗人的地方。三处实例「隔的东西」各不同、失效方式**逐字一样**：**静默漏过** —— 该红的没红，且没有任何东西报错。已升格为 §四 通用判据（「判据不得建立在『看起来像』之上」），本条是它的立项来源。
...
- **第 3 步：两把锁迁上来**（`test_text_failure_messages.py` 的 L5、`test_auth_param_used.py`）—— 从 AST 形状层落到框架账本层。**两处「换不动」如实留档，不假装覆盖**：① L5 的元数据表是③层（**意图不是事实**；失效方向是**响亮误伤**，故不配绕过型变异）；② auth 锁的「**用没用**」半边仍是②层，`route_facts` 无此能力 —— 注入 + 只 `str(user)` 不做归属校验，迁移前后**都绿**，**与缺陷 25 同形**，写进 docstring 当已知盲区。**识别**半边则落到了 0 层（依赖树 `d.call is get_current_user`，`Annotated[...]` 从静默漏过变成红）。豁免名单键从 `file:func` 改为 `(path, method)`：代码组织事实 → **框架契约事实**（文件位置与函数名可以改，路由路径是对外契约；`/api/settings/config` 因旧锁按目录 glob 而漏，新键天然覆盖，加进名单时写明它是「纯登录门、不做归属校验」）。
