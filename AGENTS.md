...
> **105–108 是 2026-09-22 新记的四条**（`users.role` 一案并入 main 后、合并前补齐项里顺带读出的），按 main 现有最大号顺延：**105 归 Spec 2、106 归蒸馏线、107 归蒸馏线、108 归 Spec 3（紧接本份）**；状态 **105 / 106 已修**（`9fae212` / `de49658`，随本份修）、**107 / 108 记账**。同样**不重算**上面那条口径行（它标着 1–98 现跑现数，这几条产生更晚）。107、108 只报告不修。
