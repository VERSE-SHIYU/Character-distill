...
- **读数（2026-09-24 现跑）**：全区间 `git diff -U0 1b09328..HEAD -- core web storage` 删掉 **130** 行 `print(`、新增 **129** 个失败记录调用（`logger.error`/`logger.warning` 124 + `nonfatal(` 5）。按级别：**ERROR（发邮件）45 处、WARNING（只上面板）84 处**。产数脚本 `tests/perf/alerting_level_census.py` —— `--end <sha>` 可回放本批各 commit 的量；它按 (文件, 档) 逐条打印，排除 `tests/` 与 `core/nonfatal.py` 的理由写在 docstring 里（后者的 `async def nonfatal(` 会被误收成一个落点）。
