...
- **三层修法（各层的判据不是同一件事，故各锁各的）**：
  ...
  - **各层的锁**：① `tests/test_pg_healthcheck.py`（5 条：负控 + **门的三个部分各一条封闭判据** + 时长解析器契约，**YAML 解析不 grep**，postgres 服务集合由镜像派生 —— 新增编排文件或改名服务自动进覆盖；负控「解析不出任何 postgres 服务」与「某个库解析不出任何消费者」防各条断言空转；「所有此类服务的整个 healthcheck 逐字相同」那条兑现「两份定义」这个取舍）② `tests/test_storage_ping.py` + `tests/test_health_ready.py` ③ `tests/test_health_probe_targets.py`。
  ...
  - **变异矩阵**：A 组 `tests/perf/pg_healthcheck_mutations.py`（**24 条全 RED 且红在预期那句**）。A-1～A-7 打**必需成分在不在**：退回 `pg_isready` / `-h` 改环回 / 删 `-h` / 口令改 `${...}` 插值 / 只改一份的 `PGCONNECT_TIMEOUT` / 服务改名而探针不改 / 锁的解析恒返回空。A-8～A-13 打**多余成分在不在**（封闭语法补的六条，每条对两个编排文件各下一处同形改动，免得先红在「多份不一致」上）：前缀加 `PGHOSTADDR=127.0.0.1` / 后加 `--host=127.0.0.1` / 末尾 `; exit 0` / 末尾 `|| true` / 结尾 `2>/dev/null` / `-h postgres` 写两遍。A-14～A-24 打**门的另外两个部分**：`disable: true` / `start_period: 10s` / `retries: 0` / `test` 首元素改 `CMD` / `test` 改字符串写法（以上打探针配置），只改一份 `interval`（打一致性），`depends_on` 退成短式列表 / `condition` 改 `service_started` / 删掉 `depends_on` 只留连接串（以上打消费者），`timeout: 5x` 与「锁的 `_duration_seconds` 解析失败时返回 0」（打解析器的契约）。B 组 `tests/perf/ping_mutations.py`（B-1～B-14，15 条）。两条驱动都**复用** `route_facts_mutations.py` 的 `_apply` / `_restore` / `_run`，不复制实现。
- **已知取舍（用户认可，照记）**：
