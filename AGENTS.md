...
- **形态锁 `tests/test_text_failure_messages.py`（5 条，不维护点位白名单）**：
  ...
  - **L5（缺陷 40 commit 二 加，扫的是 `web/routers/text.py` 而非 `text_manager.py`）**：`/api/text/upload` 的 `Form(...)` 字段除 `title` / `description` / `text_type` 三个元数据外**一个都不许有** —— 任何第四个都只能是一条新的无界正文通道。判据从**默认值的形状**推出（是不是一次 `Form(...)` 调用），不读注解。配 `test_l5_scan_is_not_vacuous` 负控（扫描必须命中 `file/title/description/text_type`）。**边界（原样保留在 docstring 里）**：它只管这一个 route 的签名，别的 route 上重开无界入参它抓不到 —— 不许说成覆盖全仓。
