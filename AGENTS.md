...
- **B′. 三处工厂手抄同一份装配 → 收敛为一处（2026-09-18，与上面 ① 是同一个病的两个显形）**
  ...
  - **证据强度（将来谁怀疑就重跑）**：① 死分支判定是**静态推出**，两个前提均为缺陷 37 已确立并有判据命令者（`llm is None ⟺ get_llm() is None`）；② 调用面**实测** —— `get_distiller(` 在 `web/` 下 9 处调用**全部传 `llm=`**，零处不传参；③ 那个条件（全仓无任何 API key）在本机结构上**复现不了** ⇒ 不是「还没验」，是**「验不了」**；而删除**可逆**（git 有记录），留着一段读起来像活的死代码才是持续成本。**「验不了」不等于「没做」** —— 这两句要分开读。
