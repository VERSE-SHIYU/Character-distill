...
- **残留（归身份线 71 批统一处理，扫描判据改为按结构判定）**：本条只做到「把 `print` 换成日志」，下面三项都不在本条的能力范围内，且都要求判据从**文本匹配**改成**按结构判定**，故留作身份线 71 批的统一做：
...
**71. web/routers 下就地 `except Exception` 只让 LLM 失败的两处放行，其余拒绝变 500** —— 状态：**已修**（2026-09-24，Spec 71 A-2 两段：首段 `1ca2329` + `97c2934`；尾段 `2292107` + `10cbb21` + `9bb536d`）
