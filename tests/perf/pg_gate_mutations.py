...
判的是 compose **真正执行的那份模型**。A 轮 24 条的**实测**去向（逐条对过，不是「都重写了」）：
  ...
  - **5 条换了承载体**：**A-7**（锁的 YAML 解析器已不存在，改由 G-21/G-22 打事实层）、
    ...
    **A-23/A-24**（`_duration_seconds` 的契约改由 `tests/test_pg_gate.py:645` 的内建断言
...
**每条对应哪条不变量**（不变量编号见 `tests/test_pg_gate.py` 的 docstring）：
  ...
  - **I3 多份定义逐字一致**：G-9 —— 只改一份是唯一在逐条检查里静默的漂移。
...
def _both(old: str, new: str) -> list[tuple]:
    ...
    I3 那条（多份定义逐字一致）在这里是**帮手**而不是干扰：两个文件同步改，那条就不会红，
...
G_GROUP = [
    ...
    ("G-9  只改 local 一份的 interval（多份定义漂移）",
     LOCK_PATH,
