...
**名单是给已有缺口用的，不是给新写的判据用的。** 方向 ① 逼人往名单里加，方向 ② 逼人删，
但**没有东西拦着「加一条今天才写的判别器」** —— 那会让名单从「已有缺口的账」变成「新债的
收容所」。所以加第三条：名单里每一条都必须存在于**本文件诞生的那个 commit**的树里
（建档 commit 现算），新判别器入名单即红，唯一出路是**当天给它写一条变异**。判据与天花板
（文本身份跨不过一次改写，与缺陷 54 同一取舍）见 `tests/test_lock_coverage.py` 里
`test_the_gap_list_only_grandfathers_discriminators_from_its_birth` 的头注。
