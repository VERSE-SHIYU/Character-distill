...
1. `storage/migrations/` 下每个 `.sql` 要么在 sqlite 的迁移次序表里，要么在
   ...
   报警** —— 先例 `079_remote_user_profiles.sql` 至今没接，SQLite 新库因此缺表。
...
_NOT_APPLIED = {
    ...
    "079_remote_user_profiles.sql": (
        "**未接线**（AGENTS.md 缺陷 21）：sqlite 次序表里没有它，SQLite 新库因此缺"
        " remote_user_profiles 表，而 sqlite_store 的 INSERT/SELECT/LEFT JOIN 已在用该表。"
        "按「新发现只记不修」留在本轮之外 —— 接线是另一件事（该表属跨区同步特性）"
    ),
}
